...
namespace Sample
{
    ...
    public class ViewModel : INotifyPropertyChanged
    {
        ...
        private void Initialize()
        {
            ...
            Stream stream = assembly.GetManifestResourceStream("Sample.Assets.Tiger.rtf");
            using (StreamReader reader = new StreamReader(stream))
            {
                animals.Add("Tiger", reader.ReadToEnd());
            }

            stream = assembly.GetManifestResourceStream("Sample.Assets.Lion.rtf");
            using (StreamReader reader = new StreamReader(stream))
            {
                animals.Add("Lion", reader.ReadToEnd());
            }

            stream = assembly.GetManifestResourceStream("Sample.Assets.Panda.rtf");
            using (StreamReader reader = new StreamReader(stream))
            {
                animals.Add("Panda", reader.ReadToEnd());
            }

            stream = assembly.GetManifestResourceStream("Sample.Assets.Bear.rtf");
            using (StreamReader reader = new StreamReader(stream))
            {
                animals.Add("Bear", reader.ReadToEnd());
            }

            stream = assembly.GetManifestResourceStream("Sample.Assets.Deer.rtf");
            using (StreamReader reader = new StreamReader(stream))
            {
